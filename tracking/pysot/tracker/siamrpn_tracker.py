...
class SiamRPNTracker(SiameseTracker):
    def __init__(self, model):
        ...
        # Another system of params for lost state
        self.flag_lost = False
#         self.lost_score_size = (LOST_INSTANCE_SIZE - cfg.TRACK.EXEMPLAR_SIZE) // \
#             cfg.ANCHOR.STRIDE + 1 + cfg.TRACK.BASE_SIZE
#         self.lost_anchors = self.generate_anchor(self.lost_score_size)
#         hanning = np.hanning(self.lost_score_size)
#         window = np.outer(hanning, hanning)
#         self.lost_window = np.tile(window.flatten(), self.anchor_num)
    ...
    def update(self, bbox):
        ...
        self.size = np.array([bbox[2] - bbox[0], bbox[3] - bbox[1]])
    ...
    def track(self, img, x_crop, scale_z, instance_size):
        """
        args:
            img(np.ndarray): BGR image
        return:
        ...
        # print(np.max(pscore))
        keep_idxs = (-pscore).argsort()[:500]
        pred_bbox = pred_bbox[:, keep_idxs]
        pscore = pscore[keep_idxs]
        ...
        import time
        t = time.time()
        keep = torchvision.ops.nms(torch.from_numpy(pred_bbox.transpose()).float().cuda(), torch.from_numpy(pscore).float().cuda(), iou_threshold=0.6)
        ...
        best_idxs = (-pscore).argsort()[:min(N, len(pscore))]
        # print(len(pscore), len(best_idxs))
        count = 0
